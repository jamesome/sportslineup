		...
		try{
			if( !oNavigator.ie ){
				if(!!oNavigator.safari && oNavigator.version <= 5){
					...
					//모바일인경우 드래그앤드랍 지원안하도록 수정 2016.12.14
				    if ( oNavigator.mobile || oNavigator.msafari || oNavigator.mopera || oNavigator.mie
				    	|| oOS.android || oOS.blackberry || oOS.ipad || oOS.iphone || oOS.mwin || oOS.nokia
				    	|| typeof g_mobile !== 'undefined' && g_mobile)
				    {
				      bSupportDragAndDropAPI = false;
				    }
				    else
    ...
    function addImage(ofile){
		...
		sFileName = cuttingNameByLength(ofile.name, 20);
     ...
     */
	 ...
	 function html5Upload()
	 {
		 ...
		 var uptime = setInterval(function(){
			 if(nImageInfoCnt == upcnt){
				 for(k=0; k < nImageInfoCnt; k++)
				 {
					 makeArrayFromString(htImageInfo['img' + k]['result']);
				 }
	...
	// function imageUpresult(result)
	// {
	// 	var upimage = [], upcount = 0;
	//
	// 	if(result != null)
	// 	{
	// 		upimage.push(result); //배열넣기
	// 		upcount ++;
	// 		console.log('a : ' + nImageInfoCnt, upcnt)
	// 		if(nImageInfoCnt == upcnt)
	// 		{
	// 			for(var k=0; k < nImageInfoCnt; k++)
	// 			{//console.log('a : ' + nImageInfoCnt, upcount)
	// 				makeArrayFromString(upimage[k]);
	// 			}
	// 		}
	// 	}
	// }

    function callAjaxForHTML5 (tempFile, sUploadURL, seqno){
			...
			onload : function(res){ // 요청이 완료되면 실행될 콜백 함수
				...
				if (res.readyState() == 4)
				{
					...
					else
					{
						...
						htImageInfo['img' + seqno]['result'] = res._response.responseText;
						//imageUpresult(htImageInfo['img' + seqno]['result']);
					}
					upcnt ++;
				}
			},
...
// 	    		oCustomEvent.stop(); 수행시 bAllowed 가 false이더라도 alert이 수행되지 않음
 	...
 	*/
 	function addUploadfiles( ev ){
 		...
 		var addTag = '';
 		if(ev.files[0] != null && ev.files[0]['name'] && ev.files[0]['size'] > 0){
	        addTag = addImage(ev.files[0]);

	        if(addTag){
	            startModeBG();
	            welDropAreaUL.prependHTML(addTag);
	            //이미지 총사이즈 view update
				updateViewTotalSize();
				//이미치 총 수 view update
				nImageInfoCnt = nImageInfoCnt+1;
				...
				goStartMode();
				// input clear
				$Element("addInputBox").$value().value = "";
	        }
	    }
 	}
